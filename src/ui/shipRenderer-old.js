...
export class ShipRenderer {
    ...
    renderCompleteShip(ship, boardElement, owner, gameState = null) {
        console.log('🚢🎨 renderCompleteShip called:', { ship: ship?.type, id: ship?.id, positions: ship?.positions?.length });
        
        if (!ship || !ship.type || !ship.positions || ship.positions.length === 0) {
            console.warn('⚠️ Invalid ship data for rendering:', ship);
            return;
        }
        ...
        if (svgElement) {
            ...
            if (ship.currentHP !== undefined && ship.maxHP !== undefined) {
                if (ship.currentHP <= 0) {
                    svgElement.classList.add('ship-sunk');
                } else if (ship.currentHP < ship.maxHP) {
        ...
        if (gameState && gameState.recentlyPlacedShip === ship.id) {
            ...
            const shipSize = this.shipSizes[ship.type];
            if (shipSize && shipSize.length > 1) {
                if (sectionIndex === 0) {
                    svgElement.classList.add('ship-section-bow');
                } else if (sectionIndex === shipSize.length - 1) {
                    svgElement.classList.add('ship-section-stern');
                } else {
                    svgElement.classList.add('ship-section-middle');
                }
            }
            ...
            // Apply hit effect if this cell was recently hit
            if (gameState && this.wasCellRecentlyHit(cellElement, gameState)) {
            ...
            // Set appropriate size for ship SVG
            ...
            // Set appropriate size based on cell dimensions
            this.scaleShipToCell(svgElement, cellElement, ship.type, isHorizontal);
        }
        
        cellElement.appendChild(shipContainer);
    }
    
    // Scale ship SVG to fit properly in cell
    scaleShipToCell(svgElement, cellElement, shipType, isHorizontal) {
        const cellRect = cellElement.getBoundingClientRect();
        const shipSize = this.shipSizes[shipType];
        
        if (cellRect.width > 0 && cellRect.height > 0) {
            if (isHorizontal) {
                // For horizontal ships, scale to fit cell width/height ratio
                svgElement.style.width = '100%';
                svgElement.style.height = 'auto';
                svgElement.style.maxHeight = '80%';
            } else {
                // For vertical ships, we need to rotate and fit differently
                svgElement.style.width = 'auto';
                svgElement.style.height = '100%';
                svgElement.style.maxWidth = '80%';
            }
        }
    }
    
    // Check if a cell was recently hit (for animation purposes)
    wasCellRecentlyHit(cellElement, gameState) {
        // This would need to be integrated with the game's hit tracking
        // For now, return false - can be enhanced later
        return false;
    }
    
    // Get ship data from game state based on cell position
    ...
    // Get the section index of a position within a ship
    getShipSectionIndex(ship, position) {
        if (!ship.positions) return 0;
        
        return ship.positions.findIndex(pos => pos.x === position.x && pos.y === position.y);
    }
    
    // Remove ship rendering from a cell
    clearShipFromCell(cellElement) {
        const shipContainer = cellElement.querySelector('.ship-container');
        if (shipContainer) {
    ...
    updateShipDamage(cellElement, ship) {
        const svgElement = cellElement.querySelector('.ship-svg');
        if (!svgElement) return;
        
        // Remove existing damage classes
        svgElement.classList.remove('ship-damaged', 'ship-sunk');
        
        // Apply appropriate damage class
        if (ship.currentHP !== undefined && ship.maxHP !== undefined) {
            if (ship.currentHP <= 0) {
                svgElement.classList.add('ship-sunk');
            } else if (ship.currentHP < ship.maxHP) {
    ...
    animateShipPlacement(cellElement) {
        const svgElement = cellElement.querySelector('.ship-svg');
        if (svgElement) {
    ...
    calculateShipDimensions(ship, cellSize, isHorizontal) {
        ...
        if (isHorizontal) {
            ...
            width = shipLength * cellSize.width;
            height = cellSize.height;
            left = minPos.x * cellSize.width;
            top = minPos.y * cellSize.height;
        } else {
            // Vertical ship spans multiple rows
            width = cellSize.width;
            height = shipLength * cellSize.height;
            left = minPos.x * cellSize.width;
            top = minPos.y * cellSize.height;
        }
    ...
    updateShipState(shipId, boardElement, newState) {
        ...
        // Apply new state
        if (newState.currentHP !== undefined && newState.maxHP !== undefined) {
            if (newState.currentHP <= 0) {
                svgElement.classList.add('ship-sunk');
            } else if (newState.currentHP < newState.maxHP) {
                svgElement.classList.add('ship-damaged');
            }
        }
